...
flyerRouter
  ...
  .post(
    ...
    [
      ...
      check('bands')
        .isLength({ min: 0, max: 60000 })
        .withMessage(`bands raw character limit is 60000`),
      check('details')
        .isLength({ min: 0, max: 60000 })
  ...
  .patch(
    ...
    [
      ...
      check('listing_state').optional()
        .custom((val, { req }) => {
          return (req.user.admin && ['Banned', 'Flagged', 'Draft', 'Private', 'Public', 'Archived'].includes(val)) || (['Draft', 'Private', 'Public', 'Archived'].includes(val))
        })
        .withMessage('Unauthorized listing control.'),
  ...
  if (creator) {
    ...
    FlyerService
          ...
          .then(flyers => {
            ...
            FlyerService
              .getTotalByCountry(knexI, country)
              .then(async count => {
